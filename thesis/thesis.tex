...
\title{
  \vspace{-1cm}
  \textbf{Quantum Gruplarının Python Ortamında Modellenmesi} \\[0.3em]
  \large{$U_q(\mathfrak{sl}_2)$ Temsil Teorisi ve $GL_q(2|1)$ için Graded Yang--Baxter Doğrulaması}
}
...
sunumun ötesine geçilerek \emph{dört orijinal hesaplamalı katkı}
sunulmuştur: \textbf{(i)} tensör çarpımı $V_m \otimes V_n$ için
Clebsch--Gordan ayrışımının en yüksek ağırlık vektörlerinin sembolik
biçimde hesaplanması (q-deforme katsayılarla); \textbf{(ii)} $\Uq$ için,
$V_1 \otimes V_1$ üzerinde $R$-matrisinin inşası, kuantum Yang--Baxter
denkleminin $8\times 8$ matris düzeyinde sıfır-kalıntı kanıtı ve örgülü
versiyonu $\check R$ için Hecke skein bağıntısının doğrulanması;
\textbf{(iii)} Çelik \& Çelik tarafından tanıtılan kuantum süpergrup
$GL_q(2|1)$ için graded Yang--Baxter doğrulaması: makalede verilen
$9\times 9$ $R$-matrisi kodlanmış, $V=\mathbb{C}^{2|1}$ baz vektörlerinin
pariteleri $p(e_1)=0,\ p(e_2)=0,\ p(e_3)=1$ alınmış, süper permütasyon
matrisi yardımıyla $R_{13}$ operatörü inşa edilmiş ve $27\times 27$ kalıntı
matrisi $Y(q)=R_{12}R_{13}R_{23}-R_{23}R_{13}R_{12}$ tüm girdiler düzeyinde
sembolik olarak sıfır doğrulanmıştır (ayrıca $V^{\otimes 4}$ üzerinde
$R_{12},R_{13},R_{14},R_{23},R_{24},R_{34}$ yerleşimleri, uzak komütativite
ve lokal Yang--Baxter kontrolleri desteklenmiştir); \textbf{(iv)} $q\to 1$
klasik limit, genel $q$ kuantum rejimi ve $q^N=1$ birim kök limitinin aynı
temsil $V_n$ üzerinde karşılaştırmalı incelenmesi (klasik $\sl_2$'nin geri
gelişi ve $V_n$'nin birim kökte indirgenebilir hâle gelmesinin sayısal
teyidi). Tüm sonuçlar \texttt{pytest} tabanlı birim testlerle
doğrulanmıştır. Çalışmanın kodu açık kaynaktır ve tezin ekinde sunulmuştur.
\end{abstract}
...
ve Çelik--Çelik tarafından tanıtılan $GL_q(2|1)$ kuantum süpergrubunu
sembolik bilgisayar cebir ortamında modellemek ve beş ana doğrulama ekseni
...
Tezin yazılım çıktısı \texttt{quantum\_group} adlı, modüler bir Python
paketidir; tüm doğrulamalar \texttt{pytest} tabanlı birim testlerle güvence
altına alınmıştır. Tüm hesaplamalar tekrarlanabilir, tüm doğrulamalar
otomatiktir.

\paragraph{Tezin yapısı.} Önce klasik teorik altyapı (gruplar, Lie cebirleri,
Hopf cebirleri, $\Uq$, temsiller) sunulur. Ardından yazılım mimarisi
özetlenir. Sonra \emph{orijinal hesaplamalı katkılar} sırasıyla verilir:
Clebsch--Gordan ayrışımı, $\Uq$ için R-matris/YBE, $GL_q(2|1)$ için graded
Yang--Baxter doğrulaması ve üç limit. Son bölüm sonuçları özetler ve ileri
çalışma yönlerini belirtir.
...
Bu deformasyon, kodda doğrudan~\eqref{eq:qint} formülü kullanılarak
modellenir:

\begin{lstlisting}[language=Python,caption={$q$-tamsayının SymPy ile modellenmesi}]
...
Bu fonksiyon klasik $n$ tamsayısının $q$-deforme karşılığı olan $\qint{n}$
ifadesini sembolik olarak üretir; $q \to 1$ limitinde $\qint{n} \to n$ elde
edilir.
...
Teorem~\ref{thm:irreps}'deki formüller, kodda doğrudan matris girişlerine
çevrilir:

\begin{lstlisting}[language=Python,caption={$V_n$ temsilinde $E,F,K,K^{-1}$ matrislerinin inşası}]
...
Bu kod, teorik formülleri doğrudan matris girişlerine dönüştürür. Böylece
soyut $\Uq$ etkisi sonlu boyutlu bir temsil üzerinde hesaplanabilir hâle
gelir.
...
\texttt{quantum\_group} paketi aşağıdaki modüllerden oluşur:
...
Test paketi (\texttt{tests/}) \texttt{pytest} tabanlı birim testlerden
oluşur; hepsi geçer.
...
otomatik olarak kanıtlamıştır. Çekirdek mantık şu biçimdedir:

\begin{lstlisting}[language=Python,caption={Tanımlayıcı bağıntıların sıfır-kalıntı yöntemiyle doğrulanması}]
...
Her bağıntı $X = Y$ biçimindedir; kod $Z = X - Y$ kalıntı matrisini kurar ve
tüm girdilerin sembolik olarak sıfır olduğunu denetler.
...
\section{Orijinal Katkı III: $GL_q(2|1)$ İçin Bilgisayar Destekli Graded Yang--Baxter Doğrulaması}

\subsection{Motivasyon}

Çelik ve Çelik \cite{CelikGL21} tarafından tanıtılan $GL_q(2|1)$ kuantum süpergrubu, $V=\mathbb{C}^{2|1}$ süper vektör uzayı üzerinde tanımlanan bir $R$-matrisi aracılığıyla RTT yöntemiyle elde edilir. Bu çalışmada, makalede verilen $9\times 9$ R-matrisinin graded Yang--Baxter denklemini sağladığı bilgisayar destekli sembolik cebir yöntemiyle doğrulanmıştır.

Bu hesap elle yapıldığında $27\times 27$ matris çarpımlarına karşılık geldiğinden uzun, tekrarlı ve hataya açıktır. Bu nedenle hesap Python/SymPy üzerinde açık baz, açık parite ve açık süper permütasyon matrisi ile modellenmiştir.

\subsection{Parite ve Süper Permütasyon}
...
\]
ile tanımlanır. Bu nedenle $P$ klasik permütasyon matrisinden yalnızca tek--tek bileşende işaret değişimiyle ayrılır. Bu tanım, kodda genel boyutta şu biçimde uygulanır:

\begin{lstlisting}[language=Python,caption={$V\otimes V$ üzerinde süper permütasyon matrisi}]
...
Bu kod $P(e_i\otimes e_j)=(-1)^{p(e_i)p(e_j)}e_j\otimes e_i$ tanımını uygular.
$GL_q(2|1)$ için $\texttt{parity}=[0,0,1]$ olduğundan yalnızca odd--odd
bileşende eksi işareti oluşur.

\subsection{R-Matrisinin Tensör Yerleşimleri}

Makalede verilen $R$-matrisi $V\otimes V$ üzerinde etki eden $9\times 9$ bir matristir. $V\otimes V\otimes V$ üzerinde
\[
...
\subsection{Sıfır-Kalıntı Doğrulaması}
...
\]
kalıntı matrisi oluşturulmuştur. Bu matris $27\times 27$ boyutludur. SymPy ile yapılan sembolik doğrulama sonucunda
\[
Y(q)_{ij}=0\qquad 1\leq i,j\leq 27
\]
olduğu tüm girdiler düzeyinde doğrulanmıştır. İlgili çekirdek kod şu
biçimdedir:

\begin{lstlisting}[language=Python,caption={$GL_q(2|1)$ için graded Yang--Baxter kalıntısı}]
...
Bu fonksiyon, $27\times 27$ boyutlu $Y(q)$ kalıntı matrisini kurar ve tüm
girdileri sembolik olarak sıfıra indirger. Dolayısıyla seçilen baz sıralaması
ve $p=(0,0,1)$ parite konvansiyonu altında graded Yang--Baxter eşitliği
matris girdileri düzeyinde doğrulanır.

\subsection{Dört Faktörlü Tensör Uzayına Genişleme}
...
\]
operatörleri $81\times 81$ matrisler olarak oluşturulabilir.
...
\section{Orijinal Katkı IV: Üç Limit -- Klasik / Kuantum / Birim Kök}
...
Bu çalışmada kuantum grup $\Uq$ ve kuantum süpergrup $GL_q(2|1)$, sembolik
bilgisayar cebir ortamında modüler bir Python paketi olarak incelenmiştir.
İnceleme beş ana doğrulama ekseni üzerinden yürütülmüştür:

\begin{itemize}[leftmargin=*]
  \item \textbf{Cebirsel doğrulama}: Dört (R1)--(R4) bağıntısı ve dokuz
        Hopf aksiyomu, $V_0, \dots, V_3$ üzerinde sembolik olarak
        kanıtlanmıştır (\texttt{pytest} tabanlı birim testler, hepsi geçer).
  ...
  \item \textbf{Üç limit}: klasik / genel $q$ / birim kök rejimleri
        ...
        gözlemlenmiştir.
  \item \textbf{Kuantum süpergrup doğrulaması}: Çelik--Çelik $GL_q(2|1)$
        R-matrisi için graded Yang--Baxter denklemi $27\times 27$ kalıntı
        matrisi düzeyinde sembolik olarak doğrulanmış; ayrıca $V^{\otimes 4}$
        üzerinde $R_{ij}$ yerleşimleri ve lokal YBE kontrolleri
        modellenmiştir.
\end{itemize}
...
# GL_q(2|1) graded Yang-Baxter dogrulamasi
from quantum_group import (
